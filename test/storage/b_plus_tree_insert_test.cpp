...
TEST(BPlusTreeTests, InsertTest6) {
  ...
  for (auto key : keys) {
    ...
    std::cout << key << " : " << rids[0].GetSlotNum() << "\n";
    int64_t value = key & 0xFFFFFFFF;
    EXPECT_EQ(rids[0].GetSlotNum(), value);
  }
...
TEST(BPlusTreeTests, InsertTest7) {
  // create KeyComparator and index schema
  auto key_schema = ParseCreateStatement("a bigint");
  GenericComparator<8> comparator(key_schema.get());
  ...
  std::vector<int64_t> delete_keys{1, 5, 3, 4};
  ...
  for (auto iterator = tree.Begin(); iterator != tree.End(); ++iterator) {
    auto location = (*iterator).second;
    EXPECT_EQ(location.GetPageId(), 0);
    EXPECT_EQ(location.GetSlotNum(), 2);
  }
  std::cout << "end\n";
  bpm->UnpinPage(HEADER_PAGE_ID, true);
...
TEST(BPlusTreeTests, scaletest1) {
  ...
  std::vector<int64_t> delete_keys{1, 2, 3, 4};
  for (auto key : delete_keys) {
    int64_t value = key & 0xFFFFFFFF;
    rid.Set(static_cast<int32_t>(key >> 32), value);
    index_key.SetFromInteger(key);
  ...
  for (auto iterator = tree.Begin(); iterator != tree.End(); ++iterator) {
    auto location = (*iterator).second;
    EXPECT_EQ(location.GetPageId(), 0);
    EXPECT_EQ(location.GetSlotNum(), start++);
  }
  std::cout << "end\n";
  bpm->UnpinPage(HEADER_PAGE_ID, true);
