  ...
  auto Compute(const std::string &val) const -> std::string {
    ...
    switch (expr_type_) {
      case StringExpressionType::Lower:
        std::transform(val.cbegin(), val.cend(), std::back_inserter(temp), [](char c) { return tolower(c); });
        break;
      case StringExpressionType::Upper:
        std::transform(val.cbegin(), val.cend(), std::back_inserter(temp), [](char c) { return toupper(c); });
        break;
    }
    return temp;
  }
