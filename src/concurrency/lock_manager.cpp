...
auto LockManager::LockTable(Transaction *txn, LockMode lock_mode, const table_oid_t &oid) -> bool {
  ...
  switch (txn->GetIsolationLevel()) {
    case IsolationLevel::REPEATABLE_READ: {
      if (txn->GetState() == TransactionState::SHRINKING) {  // NOLINT
        ThrowAbort(txn, AbortReason::LOCK_ON_SHRINKING);
      }
    } break;
    case IsolationLevel::READ_COMMITTED: {
      if (txn->GetState() == TransactionState::SHRINKING &&                               // NOLINT
          !(lock_mode == LockMode::SHARED || lock_mode == LockMode::INTENTION_SHARED)) {  // NOLINT
        ThrowAbort(txn, AbortReason::LOCK_ON_SHRINKING);
      }
    } break;
    case IsolationLevel::READ_UNCOMMITTED: {
      // required
      if (!(lock_mode == LockMode::EXCLUSIVE || lock_mode == LockMode::INTENTION_EXCLUSIVE)) {  // NOLINT
        ThrowAbort(txn, AbortReason::LOCK_SHARED_ON_READ_UNCOMMITTED);
      }
      // allowed
      if (!(txn->GetState() == TransactionState::GROWING &&                                       // NOLINT
            (lock_mode == LockMode::EXCLUSIVE || lock_mode == LockMode::INTENTION_EXCLUSIVE))) {  // NOLINT
        ThrowAbort(txn, AbortReason::LOCK_SHARED_ON_READ_UNCOMMITTED);
      }
    } break;
  }
  ...
  for (auto it = lock_request_queue->request_queue_.begin(); it != lock_request_queue->request_queue_.end(); it++) {
    ...
    if (lock_request->txn_id_ == txn->GetTransactionId()) {
      ...
      if (!((old_lock_mode == LockMode::INTENTION_SHARED) ||                                                 // NOLINT
            (old_lock_mode == LockMode::SHARED &&                                                            // NOLINT
             (lock_mode == LockMode::SHARED_INTENTION_EXCLUSIVE || lock_mode == LockMode::EXCLUSIVE)) ||     // NOLINT
            (old_lock_mode == LockMode::INTENTION_EXCLUSIVE &&                                               // NOLINT
             (lock_mode == LockMode::SHARED_INTENTION_EXCLUSIVE || lock_mode == LockMode::EXCLUSIVE)) ||     // NOLINT
            (old_lock_mode == LockMode::SHARED_INTENTION_EXCLUSIVE && lock_mode == LockMode::EXCLUSIVE))) {  // NOLINT
...
auto LockManager::LockRow(Transaction *txn, LockMode lock_mode, const table_oid_t &oid, const RID &rid) -> bool {
  ...
  for (auto it = lock_request_queue->request_queue_.begin(); it != lock_request_queue->request_queue_.end(); it++) {
    ...
    if (lock_request->txn_id_ == txn->GetTransactionId()) {
      ...
      LockMode old_lock_mode = lock_request->lock_mode_;
      if (!((old_lock_mode == LockMode::INTENTION_SHARED) ||                                                 // NOLINT
            (old_lock_mode == LockMode::SHARED && lock_mode != LockMode::INTENTION_SHARED &&                 // NOLINT
             lock_mode != LockMode::INTENTION_EXCLUSIVE) ||                                                  // NOLINT
            (old_lock_mode == LockMode::INTENTION_EXCLUSIVE &&                                               // NOLINT
             (lock_mode == LockMode::SHARED_INTENTION_EXCLUSIVE || lock_mode == LockMode::EXCLUSIVE)) ||     // NOLINT
            (old_lock_mode == LockMode::SHARED_INTENTION_EXCLUSIVE && lock_mode == LockMode::EXCLUSIVE))) {  // NOLINT
        ThrowAbort(txn, AbortReason::INCOMPATIBLE_UPGRADE);
...
auto LockManager::GrantAllowed(Transaction *txn, const std::shared_ptr<LockRequestQueue> &lock_request_queue,
  ...
  std::cout << "unexpected when grant-allow\n";
  return false;
}
...
auto LockManager::Dfs(std::vector<txn_id_t> visited, std::vector<txn_id_t> keys, txn_id_t *abort_txn_id) -> bool {
  ...
  for (const auto &key : keys) {
    ...
    if (Dfs(visited, wait_keys, abort_txn_id)) {
      return true;
    }
    visited.pop_back();
  }
